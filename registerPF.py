...
class App(ctk.CTk):
    ...
    def system(self):
        ...
        styles.registerBtnPF(self, frame)
    ...
    def Register(self):
        ...
        try:
            query_check = "SELECT 1 FROM users WHERE email = %s"
            cursor.execute(query_check, (email,))
            email_exists = cursor.fetchone() is not None

            query_check_username = "SELECT 1 FROM users WHERE usernames = %s"
            ...
            if email_exists:
                ...
                messagebox.showerror("Erro", "Já existe um usuário com este username")
            
            else:
                query = "INSERT INTO users (usernames, password, email, userType) VALUES (%s, %s, %s, 'PF')"
                cursor.execute(query, (username, password, email))
                db.commit()
                messagebox.showinfo("Sucesso", "Usuário registrado com sucesso!")
                self.destroy()
                subprocess.run(['python', 'LoginPF.py', username])
